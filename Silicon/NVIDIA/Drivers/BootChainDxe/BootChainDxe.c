  ...
  )
{
  ...
  Status = gRT->SetVariable (Variable->Name,
                             Variable->Guid,
                             Variable->Attributes,
                             0,
                             NULL);
  ...
  )
{
  ...
  *Value    = 0;
  Size      = Variable->Bytes;
  Status    = gRT->GetVariable (Variable->Name,
                                Variable->Guid,
                                NULL,
                                &Size,
                                Value);
  ...
  DEBUG ((DEBUG_INFO, "%a: Read %s=%u: %r\n",
          __FUNCTION__, Variable->Name,
          (!EFI_ERROR (Status)) ? *Value : 255, Status));
  ...
  )
{
  ...
  Variable = &mBCVariables[VariableIndex];
  DEBUG ((DEBUG_INFO, "%a: Setting %s=%u\n",
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Error setting %s to %u: %r\n",
            __FUNCTION__, Variable->Name, Value, Status));
  }
  ...
  )
{
  ...
  if (Status == EFI_NOT_FOUND) {
    ...
    Size = 0;
    BootNextStatus = gRT->GetVariable (L"BootNext",
                               &gEfiGlobalVariableGuid,
                               NULL,
                               &Size,
                               NULL);
    if (BootNextStatus != EFI_NOT_FOUND) {
      DEBUG ((DEBUG_ERROR, "%a: BootNext exists, failing boot chain update\n",
              __FUNCTION__));
  ...
  if (BCResetCount >= BOOT_CHAIN_MAX_RESET_COUNT) {
    DEBUG ((DEBUG_ERROR, "%a: Max resets attempted, failing update\n",
            __FUNCTION__));
  ...
  DEBUG ((DEBUG_INFO, "%a: updating BCT to new BootChain=%u\n",
          __FUNCTION__, BCNext));
  ...
  DEBUG ((DEBUG_INFO, "%a: Resetting to boot chain=%u, status=%u, reset count=%u\n",
          __FUNCTION__, BCNext, BCStatus, BCResetCount));

  Print (L"Rebooting to new boot chain\n\r");
  BootChainReset (BCNext);

SetStatusAndBootOs:
  DEBUG ((DEBUG_INFO, "%a: Setting status=%u before booting OS\n",
...
BootOs:
  if ((BCStatus == MAX_UINT32) && BrBctUpdateNeeded ()) {
    DEBUG ((DEBUG_INFO, "%a: BrBctUpdateNeeded, new BootChain=%u\n",
            __FUNCTION__, mBootChain));

    Status = mBrBctUpdateProtocol->UpdateFwChain (mBrBctUpdateProtocol,
                                                  mBootChain);
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "%a: error updating BrBct to BootChain=%u\n",
              __FUNCTION__, mBootChain));
    }
  }

  DEBUG ((DEBUG_INFO, "%a: Booting OS, FW BootChain=%u, Status=%d\n",
          __FUNCTION__, mBootChain, BCStatus));
  ...
  )
{
  ...
  if (BCStatus != STATUS_SUCCESS) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "%a: Failed to get Previous: %r\n",
              __FUNCTION__, Status));
      DEBUG ((DEBUG_ERROR, "%a: Unable to determine original boot chain\n",
              __FUNCTION__, Status));
      goto Cleanup;
    }
...
Cleanup:
  ...
  if (RebootOriginalBootChain) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "%a: Failed to update BR-BCT boot chain: %r\n",
              __FUNCTION__, Status));
      return;
    }

    DEBUG ((DEBUG_INFO, "%a: Doing reset to restore original boot chain=%u\n",
            __FUNCTION__, BCPrevious));
  ...
  )
{
  ...
  if ((Hob != NULL) &&
    ...
    DEBUG ((DEBUG_ERROR, "%a: Error getting resource info\n",
            __FUNCTION__));
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: BrBctUpdate Protocol Guid=%g not found: %r\n",
            __FUNCTION__, &gNVIDIABrBctUpdateProtocolGuid, Status));
    goto Done;
  }
