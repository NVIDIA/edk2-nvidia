  ...
  )
{
  ...
  Status = gMmst->MmLocateProtocol (&gNVIDIANorFlashProtocolGuid,
                                NULL,
  ...
  for (Index = 0; Index < FVB_TO_CREATE; Index++) {
    ...
    Status = gMmst->MmInstallProtocolInterface(&FvpData[Index].Handle,
                                               &gEfiSmmFirmwareVolumeBlockProtocolGuid,
                                               EFI_NATIVE_INTERFACE,
                                               &FvpData[Index].FvbProtocol);
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "%a: Failed to install FVP protocol Index %d %x Status %r\r\n", __FUNCTION__, Index,
                                                                                        &FvpData[Index].Handle, Status));
      return Status;
    }
  }

  Status = gMmst->MmInstallProtocolInterface(&ImageHandle,
