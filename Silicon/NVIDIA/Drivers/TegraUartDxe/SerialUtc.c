/** @file
  UTC Serial driver that layers on top of a Serial Port Library instance.
  ...
  )
{
  ...
  Private = SERIAL_UTC_IO_PRIVATE_DATA_FROM_PROTOCOL (This);
  ...
  )
{
  ...
  Private->Signature                  = SERIAL_UTC_IO_SIGNATURE;
  Private->TegraUartObj               = TegraUtcSerialPortGetObject ();
