...
#include "Th500PcieControllerPrivate.h"
  ...
  )
{
  ...
  if ((Buffer == NULL) ||
    ...
    if (((PciAddress.Bus == This->MinBusNumber) ||
      ...
      ConfigAddress += PciAddress.Device << 15;
      ConfigAddress += PciAddress.Function << 12;
      if (ConfigAddress >= (Private->ConfigurationSpace + Private->ConfigurationSize)) {
        if (Read) {
          SetMem (Buffer, Length, 0xFF);
          Status = EFI_SUCCESS;
        } else {
