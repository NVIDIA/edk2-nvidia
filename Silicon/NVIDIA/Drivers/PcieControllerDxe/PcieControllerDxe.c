    ...
    case DeviceDiscoveryDriverBindingStart:
      ...
      for (RangeIndex = 0; RangeIndex < NumberOfRanges; RangeIndex++) {
        ...
        if (Space == PCIE_DEVICETREE_SPACE_IO) {
          ...
          if (Prefetchable) {
            RootBridge->PMem.Base        = DeviceAddress;
            RootBridge->PMem.Limit       = Limit;
            RootBridge->PMem.Translation = Translation;
            // if both prefetch and non-prefetch are present, assume they are contiguous
            Private->MemBase = (Private->MemBase == 0) ? HostAddress :
                               MIN (Private->MemBase, HostAddress);
            Private->MemLimit = (Private->MemLimit == 0) ? HostAddress + Size - 1 :
                                MAX (Private->MemLimit, HostAddress + Size - 1);
            Private->AddressMapInfo[Private->AddressMapCount].SpaceCode = 3;
            DEBUG ((DEBUG_INFO, "PREF32: DevAddr = 0x%lX Limit = 0x%lX Trans = 0x%lX MemBase = 0x%lX, MemLimit = 0x%lX\n", DeviceAddress, Limit, Translation, Private->MemBase, Private->MemLimit));
          } else {
            RootBridge->Mem.Base        = DeviceAddress;
            ...
            Private->MemLimit = (Private->MemLimit == 0) ? HostAddress + Size - 1 :
                                MAX (Private->MemLimit, HostAddress + Size - 1);
            Private->AddressMapInfo[Private->AddressMapCount].SpaceCode = 3;
            DEBUG ((DEBUG_INFO, "MEM32: DevAddr = 0x%lX Limit = 0x%lX Trans = 0x%lX MemBase = 0x%lX, MemLimit = 0x%lX\n", DeviceAddress, Limit, Translation, Private->MemBase, Private->MemLimit));
          }
        } else {
