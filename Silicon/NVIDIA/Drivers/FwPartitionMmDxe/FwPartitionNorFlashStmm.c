  ...
  )
{
  ...
  if ((Offset % EraseBlockSize) != 0) {
    DEBUG ((DEBUG_ERROR, "%a: unaligned erase, block size=%u, Offset=%llu\n",
            __FUNCTION__, EraseBlockSize, Offset));
  ...
  )
{
  ...
  DEBUG ((DEBUG_VERBOSE, "%a: ENTRY  offset=%llu, bytes=%u\n",
  ...
  )
{
  ...
  Status = gMmst->MmLocateHandle (ByProtocol,
  ...
  for (Index = 0; Index < NumHandles; Index++) {
    ...
    Status = gMmst->MmHandleProtocol (Handle,
                                    &gNVIDIANorFlashProtocolGuid,
                                    (VOID **) &NorFlash);
    if (EFI_ERROR (Status) || (NorFlash == NULL)) {
      DEBUG ((DEBUG_ERROR, "Failed to get MM-NorFlash for handle index %u: %r\n",
              Index, Status));
      continue;
    }
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "MM-NorFlash attributes for handle %u failed: %r\n",
              Index, Status));
      continue;
    }

    DEBUG ((DEBUG_INFO, "Found MM-NorFlash BlockSize=%u, MemoryDensity=%llu\n",
            Attributes.BlockSize, Attributes.MemoryDensity));
