  ...
  )
{
  ...
  DEBUG ((DEBUG_VERBOSE, "%a: read %s Offset=%u, Bytes=%u\n",
          __FUNCTION__, MmInfo->PartitionName, Offset, Bytes));
  ...
  )
{
  ...
  DEBUG ((DEBUG_VERBOSE, "%a: write %s Offset=%u, Bytes=%u\n",
          __FUNCTION__, MmInfo->PartitionName, Offset, Bytes));
  ...
  )
{
  ...
  Status = MmSendGetPartitions (MAX_FW_PARTITIONS,
                                PartitionInfoBuffer,
                                &Count,
                                BrBctEraseBlockSize);
  if (EFI_ERROR (Status)) {
  ...
  for (Index = 0; Index < Count; Index++) {
    ...
    Status = FwPartitionAdd (PartitionInfo->Name,
                             DeviceInfo,
                             0,
                             PartitionInfo->Bytes);
    if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  Status = MmSendInitialize (ActiveBootChain,
                             PcdOverwriteActiveFwPartition);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "%a: Error initializing MM interface: %r\n",
            __FUNCTION__, Status));
    goto Done;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "%a: Error initializing MM devices: %r\n",
            __FUNCTION__, Status));
    goto Done;
  }
