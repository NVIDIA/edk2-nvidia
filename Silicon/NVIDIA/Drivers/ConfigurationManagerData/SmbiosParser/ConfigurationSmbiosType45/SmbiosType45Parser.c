  ...
  )
{
  ...
  if (*PrivateInfoSet != NULL) {
    for (HandleIndex = 0; HandleIndex < NumHandles; HandleIndex++) {
      ...
      FREE_NON_NULL (PrivateInfoSetPtr->ImageInfoHead);
      FREE_NON_NULL (PrivateInfoSetPtr->PackageVersionName);
      FREE_NON_NULL (PrivateInfoSetPtr->DevicePathString);
    }
  ...
  )
{
  ...
  if (NewFirmwareInventoryInfo == NULL) {
    DEBUG ((DEBUG_ERROR, "%a: Unable to reallocate space for a new element in FirmwareInventoryInfo\n", __FUNCTION__));
  ...
  )
{
  ...
  for (HandleIndex = 0; HandleIndex < NumFmpHandles; HandleIndex++) {
    ...
    if (NewFirmwareInventoryInfo == NULL) {
      Status = EFI_OUT_OF_RESOURCES;
      DEBUG ((DEBUG_ERROR, "%a: Unable to reallocate space for a new component in FirmwareInventoryInfo\n", __FUNCTION__));
      goto Exit;
    }
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    BiosInfo = (CM_SMBIOS_BIOS_INFO *)DescPtr->Data;
  } else {
    DEBUG ((DEBUG_ERROR, "%a: Got %r trying to find the BiosInfo \n", __FUNCTION__, Status));
  }

  Status = NvFindEntry (ParserHandle, CREATE_CM_SMBIOS_OBJECT_ID (ESmbiosObjSystemSlotInfo), CM_NULL_TOKEN, &DescPtr);
  if (!EFI_ERROR (Status)) {
    SystemSlotInfo = (CM_SMBIOS_SYSTEM_SLOTS_INFO *)DescPtr->Data;
    NumSystemSlots = DescPtr->Count;
  } else {
  ...
  if (NumFirmwareComponents > 0) {
    ...
    //
    Desc.ObjectId = CREATE_CM_SMBIOS_OBJECT_ID (ESmbiosObjFirmwareInventoryInfo);
    Desc.Size     = NumFirmwareComponents * sizeof (CM_SMBIOS_FIRMWARE_INVENTORY_INFO);
    Desc.Count    = NumFirmwareComponents;
    Desc.Data     = FirmwareInventoryInfo;
...
CleanupAndReturn:
  FREE_NON_NULL (FirmwareInventoryInfo);
  return Status;
}
