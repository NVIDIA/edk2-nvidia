  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Unable to allocate a token for SMBIOS Type 0: %r\n", __FUNCTION__, Status));
    goto CleanupAndReturn;
  }

  BiosInfo->BiosInfoToken = TokenMap[0];

  // Add the CmObj to the Configuration Manager.
  Desc.ObjectId = CREATE_CM_SMBIOS_OBJECT_ID (ESmbiosObjBiosInfo);
  Desc.Size     = sizeof (CM_SMBIOS_BIOS_INFO);
  Desc.Count    = 1;
  Desc.Data     = BiosInfo;
  Status        = NvAddMultipleCmObjWithTokens (ParserHandle, &Desc, TokenMap, CM_NULL_TOKEN);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Unable to add Smbios Type 0 to ConfigManager: %r\n", __FUNCTION__, Status));
    goto CleanupAndReturn;
  }
