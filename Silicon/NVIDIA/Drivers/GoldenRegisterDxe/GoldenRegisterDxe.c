  ...
  )
{
  ...
  if (Private->GrOutSize < (sizeof (GR_DATA_HEADER) + DataHeader->Mb1Size + DataHeader->Mb2Size)) {
    DEBUG ((DEBUG_ERROR, "%a: GR output buffer is likely corrupted!\r\n", __FUNCTION__));
    return;
  }

  if ((Private->NumberOfAddresses * sizeof (GR_DATA)) > (Private->GrOutSize - (sizeof (GR_DATA_HEADER) + DataHeader->Mb1Size + DataHeader->Mb2Size))) {
    DEBUG ((DEBUG_ERROR, "%a: Too many addresses to read, %d elements can't fit in %d bytes!\r\n", __FUNCTION__, Private->NumberOfAddresses, Private->GrOutSize));
    return;
  }

  GrData = (GR_DATA *)(Private->GrOutBase + sizeof (GR_DATA_HEADER) + DataHeader->Mb1Size + DataHeader->Mb2Size);

  for (Count = 0; Count < Private->NumberOfAddresses; Count++) {
    GrData->Address = Private->Address[Count];
    GrData->Data    = *(UINT32 *)(UINTN)(Private->Address[Count]);
    DEBUG ((DEBUG_ERROR, "UEFI GR Dump: Address: 0x%x Data: 0x%x\n", GrData->Address, GrData->Data));
    GrData++;
  }

  DataHeader->UefiOffset = DataHeader->Mb2Offset + DataHeader->Mb2Size;
  DataHeader->UefiSize   = Private->NumberOfAddresses * sizeof (GR_DATA);
  ...
  )
{
  ...
  Private->NumberOfAddresses = Size / sizeof (UINT32);
