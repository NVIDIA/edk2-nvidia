...
)
{
  ...
  for (Index = 0; (Index < PassThruNumHandles) && (EmmcFound == FALSE); Index++) {
    Status = gBS->HandleProtocol (PassThruHandleBuffer[Index],
                                  &gEfiSdMmcPassThruProtocolGuid,
                                  (VOID **) &PassThru);
    if (EFI_ERROR (Status) || (PassThru == NULL)) {
      DEBUG ((DEBUG_INFO, "Failed to get PassThru for handle index %u: %r\n",
              Index, Status));
      continue;
    }
...
)
{
  ...
  DEBUG ((DEBUG_INFO, "Manufac:0x%x ProductName:0x%x:0x%x:0x%x:0x%x:0x%x \n", Cid->ManufacturerId,
          Cid->ProductName[0], Cid->ProductName[1], Cid->ProductName[2],
          Cid->ProductName[3], Cid->ProductName[4], Cid->ProductName[5]));
...
)
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a [0]:Failed to set BlockCount %d (%r)\n",
            __FUNCTION__, NumReqFrames, Status));
    return Status;
  }

  Status = RpmbWriteBlocks (PassThru, Slot, NumReqFrames, ReqFrame);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a [1]:Failed to Send Write Res %r\n",
            __FUNCTION__, Status));
    return Status;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a [2]:Failed to set BlockCount 1 %r\n",
            __FUNCTION__, Status));
    return Status;
  }
  ...
  Status = RpmbSetBlockCount (PassThru, Slot, 1, FALSE);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a [4]:Failed to set BlockCount 1 %r\n",
            __FUNCTION__, Status));
    return Status;
  }
...
)
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "[2]:Failed to set BlockCount %d %r\n",
            NumRespFrames, Status));
    return Status;
  }
  ...
  switch (Request) {
    case RPMB_MSG_TYPE_REQ_WRITE_COUNTER_VAL_READ:
      if ((NumReqFrames != 1) || (NumRespFrames != 1)) {
        DEBUG ((DEBUG_ERROR, "Invalid NumFrames (Resp %d Req %d) for Cmd %d\n",
                NumRespFrames, NumReqFrames, Request));
        Status =  EFI_INVALID_PARAMETER;
        goto Error;
      }
      ZeroMem(RespFrame, RPMB_FRAME_SIZE);
...
)
{
  ...
  if (Msg->NumParams !=2 ||
    ...
    DEBUG ((DEBUG_INFO, "Invalid RPMB Params Num %d [0].Attr %d [1].Attr %d\n",
  ...
  DEBUG ((DEBUG_INFO, "ReqShm Addr %lx Size %ld Offset %ld Buf %lx\n",
          Msg->Params[0].Union.RMemory.SharedMemoryReference, ReqSize,
          Msg->Params[0].Union.RMemory.Offset, (UINT64)ReqBuf));
  DEBUG ((DEBUG_INFO, "RespShm Addr %lx Size %ld Offset %ld Buf %lx\n",
          Msg->Params[1].Union.RMemory.SharedMemoryReference, RespSize,
          Msg->Params[1].Union.RMemory.Offset, (UINT64) RespBuf));
  ...
  if (EmmcPassThru == NULL) {
    ...
    if (EFI_ERROR(Status)) {
      DEBUG ((DEBUG_ERROR, "Failed to Locate EMMC PassThrough Protocol:%r\n",
              Status));
      return;
    }
  }
  switch (RpmbReq->Cmd) {
    ...
    break;
    case RPMB_DATA_REQ:
    Status = HandleRpmbDataReq (EmmcPassThru, EmmcSlot,
                  ((VOID *)((RPMB_REQUEST *)(RpmbReq) + 1)),
                  (ReqSize - sizeof(RPMB_REQUEST)), RespBuf, RespSize);
    if (EFI_ERROR(Status)) {
