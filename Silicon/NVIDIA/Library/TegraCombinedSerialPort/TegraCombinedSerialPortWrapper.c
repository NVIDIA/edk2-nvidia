/** @file
  Serial I/O Port library wrapper for TegraCombinedSerialPortLib.
  This provides the standard SerialPortLib interface by wrapping
  TegraCombinedSerialPortLib functions.
...
// External functions from TegraCombinedSerialPortLib
extern TEGRA_UART_OBJ *
TegraCombinedSerialPortGetObject (
  VOID
  );

// Since the Tegra functions require a SerialBaseAddress but the standard
// SerialPortLib doesn't provide one, we use 0 as a placeholder
#define TEGRA_SERIAL_BASE_ADDRESS  0
  ...
  )
{
  if (mTegraUartObj == NULL) {
    mTegraUartObj = TegraCombinedSerialPortGetObject ();
    if (mTegraUartObj == NULL) {
