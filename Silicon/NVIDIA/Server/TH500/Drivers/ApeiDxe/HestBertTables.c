...
SetupNotificationStructure (
  ...
  IN OUT EFI_ACPI_6_6_HARDWARE_ERROR_NOTIFICATION_STRUCTURE  *NotificationStructure
  )
{
  NotificationStructure->Type = ErrorSource->NotificationType;

  switch (NotificationStructure->Type) {
    case EFI_ACPI_6_6_HARDWARE_ERROR_NOTIFICATION_SOFTWARE_DELEGATED_EXCEPTION:
      NotificationStructure->Vector = ErrorSource->SourceIdSdei;
      break;

    case EFI_ACPI_6_6_HARDWARE_ERROR_NOTIFICATION_GSIV:
      NotificationStructure->Vector = TH500_SW_IO2_INTR;
      break;

    case EFI_ACPI_6_6_HARDWARE_ERROR_NOTIFICATION_POLLED:
...
{
  EFI_ACPI_6_6_HARDWARE_ERROR_SOURCE_TABLE_HEADER                 *HestTable;
  UINTN                                                           HestTableSize;
  UINT8                                                           Checksum;
  UINTN                                                           AcpiTableHandle;
  UINTN                                                           i;
  EFI_ACPI_6_6_GENERIC_HARDWARE_ERROR_SOURCE_VERSION_2_STRUCTURE  GHESv2Instance;
  ...
  HestTableSize = sizeof (EFI_ACPI_6_6_HARDWARE_ERROR_SOURCE_TABLE_HEADER);

  for (i = 0; i < TotalNumErrorSource; i++) {
    if (ErrorSource[i].GhesType == EFI_ACPI_6_6_GENERIC_HARDWARE_ERROR_VERSION_2) {
      if (ErrorSource[i].EventId != BERT_EVENT_ID) {
        /* Each error source can lead to 2 entries to support dynamic GSIV/SDEI */
        HestTableSize += sizeof (EFI_ACPI_6_6_GENERIC_HARDWARE_ERROR_SOURCE_VERSION_2_STRUCTURE) * 2;
      }
    } else {
  ...
  *HestTable = (EFI_ACPI_6_6_HARDWARE_ERROR_SOURCE_TABLE_HEADER) {
    .Header = {
      .Signature       = EFI_ACPI_6_6_HARDWARE_ERROR_SOURCE_TABLE_SIGNATURE,
      .Length          = sizeof (EFI_ACPI_6_6_HARDWARE_ERROR_SOURCE_TABLE_HEADER),
      .Revision        = EFI_ACPI_6_6_HARDWARE_ERROR_SOURCE_TABLE_REVISION,
  ...
  ErrorSourceStructure        = ((UINT8 *)HestTable + sizeof (EFI_ACPI_6_6_HARDWARE_ERROR_SOURCE_TABLE_HEADER));
  ...
  for (i = 0; i < TotalNumErrorSource; i++) {
    ...
    ZeroMem (&GHESv2Instance, sizeof (EFI_ACPI_6_6_GENERIC_HARDWARE_ERROR_SOURCE_VERSION_2_STRUCTURE));

    GHESv2Instance = (EFI_ACPI_6_6_GENERIC_HARDWARE_ERROR_SOURCE_VERSION_2_STRUCTURE) {
    ...
    ErrorSourceStructure     += sizeof (EFI_ACPI_6_6_GENERIC_HARDWARE_ERROR_SOURCE_VERSION_2_STRUCTURE);
    HestTable->Header.Length += sizeof (EFI_ACPI_6_6_GENERIC_HARDWARE_ERROR_SOURCE_VERSION_2_STRUCTURE);
     ...
     */
    if ((ErrorSource[i].NotificationType == EFI_ACPI_6_6_HARDWARE_ERROR_NOTIFICATION_GSIV) &&
        (ErrorSource[i].SourceIdSdei != 0))
    {
      ErrorSource[i].NotificationType = EFI_ACPI_6_6_HARDWARE_ERROR_NOTIFICATION_SOFTWARE_DELEGATED_EXCEPTION;
...
{
  EFI_STATUS                                   Status = EFI_SUCCESS;
  EFI_ACPI_6_6_BOOT_ERROR_RECORD_TABLE_HEADER  *BertTable;
  UINTN                                        BertSize;
  UINT8                                        Checksum;
  UINTN                                        AcpiTableHandle;
  UINTN                                        i;
  UINT32                                       TotalNumErrorSource;
  EFI_ACPI_6_6_GENERIC_ERROR_STATUS_STRUCTURE  *Gess;
  EFI_ACPI_TABLE_PROTOCOL                      *AcpiTableProtocol   = NULL;
  ...
  Gess                    = (EFI_ACPI_6_6_GENERIC_ERROR_STATUS_STRUCTURE *)*BootErrorRegionRegister;

  /* Allocate enough space for the header and error sources */
  BertSize  = sizeof (EFI_ACPI_6_6_BOOT_ERROR_RECORD_TABLE_HEADER);
  BertTable = AllocateReservedZeroPool (BertSize);

  *BertTable = (EFI_ACPI_6_6_BOOT_ERROR_RECORD_TABLE_HEADER) {
    .Header = {
      .Signature       = EFI_ACPI_6_6_BOOT_ERROR_RECORD_TABLE_SIGNATURE,
      .Length          = sizeof (EFI_ACPI_6_6_BOOT_ERROR_RECORD_TABLE_HEADER),
      .Revision        = EFI_ACPI_6_6_BOOT_ERROR_RECORD_TABLE_REVISION,
      .OemTableId      = PcdGet64 (PcdAcpiDefaultOemTableId),
      .OemRevision     = EFI_ACPI_OEM_REVISION,
      .CreatorId       = EFI_ACPI_CREATOR_ID,
      .CreatorRevision = EFI_ACPI_CREATOR_REVISION
    ...
    .BootErrorRegionLength = Gess->DataLength + sizeof (EFI_ACPI_6_6_GENERIC_ERROR_STATUS_STRUCTURE)
  };
