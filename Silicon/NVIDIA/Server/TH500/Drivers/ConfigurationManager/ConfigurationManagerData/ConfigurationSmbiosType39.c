  ...
  )
{
  ...
  for (Psu = 0; Psu < PsuCount; Psu++) {
    ...
    PsuInfo[Psu].PowerSupplyInfoToken = REFERENCE_TOKEN (PsuInfo[Psu]);
  }
  ...
  //
  Repo->CmObjectId    = CREATE_CM_SMBIOS_OBJECT_ID (ESmbiosObjPowerSupplyInfo);
  Repo->CmObjectToken = CM_NULL_TOKEN;
  Repo->CmObjectSize  = sizeof (CM_SMBIOS_POWER_SUPPLY_INFO) * PsuCount;
  Repo->CmObjectCount = PsuCount;
  Repo->CmObjectPtr   = PsuInfo;
  Repo++;
