/** @file
This module installs the SMBIOS data tables for T234.
...
**/
STATIC UINTN NvGetCpuFreqHz ( VOID  )
{
  /*
   * Temporarily set to return a fixed frequency.
   * A permanent solution is to read the NV-SOC/ARM PMU Counters
...
{
  MiscProcessorData->CurrentSpeed = NvGetCpuFreqHz();
  MiscProcessorData->MaxSpeed     = NvGetCpuFreqHz();
...
)
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    /* Type2 Table Data */
    SkuSize = (sizeof (T234CvmEeprom->Sku) + 1) * sizeof (CHAR16);
    ...
    UnicodeSPrint(MiscData->BoardSku,
                  SkuSize,
                  L"%x", T234CvmEeprom->Sku);

    VersionSize = (sizeof (T234CvmEeprom->Version) + 1) * sizeof (CHAR16);
    ...
    UnicodeSPrint(MiscData->BoardVersion,
                  VersionSize,
                  L"%u", T234CvmEeprom->Version);

    SnSize = (sizeof (T234CvmEeprom->SerialNumber) + 1) * sizeof(CHAR16);
    ...
    for (c = 0; c <  sizeof (T234CvmEeprom->SerialNumber); c++) {
      UnicodeSPrint(&MiscData->BoardSerialNumber[c],
                    SnSize,
                    L"%c", T234CvmEeprom->SerialNumber[c]);
    }
    ...
    CharCount = 0;
    for (c = 0; c <  sizeof (T234CvmEeprom->PartNumber.Leading); c++) {
      UnicodeSPrint(&MiscData->BoardAssetTag[CharCount++],
                    AssetTagSize,
                    L"%c", T234CvmEeprom->PartNumber.Leading[c]);
    }

    AssetTagSize -= (c * sizeof (CHAR16));
    UnicodeSPrint(&MiscData->BoardAssetTag[CharCount++],
                  AssetTagSize,
                  L"%c", T234CvmEeprom->PartNumber.Separator0);

    AssetTagSize -= sizeof (CHAR16);
    UnicodeSPrint(&MiscData->BoardAssetTag[CharCount++],
                  AssetTagSize,
                  L"%c", T234CvmEeprom->PartNumber.Class);

    AssetTagSize -= sizeof (CHAR16);
    for (c = 0; c <  sizeof (T234CvmEeprom->PartNumber.Id); c++) {
      UnicodeSPrint(&MiscData->BoardAssetTag[CharCount++],
                    AssetTagSize,
                    L"%c", T234CvmEeprom->PartNumber.Id[c]);
    }

    AssetTagSize -= (c * sizeof (CHAR16));
    UnicodeSPrint(&MiscData->BoardAssetTag[CharCount++],
                  AssetTagSize,
                  L"%c", T234CvmEeprom->PartNumber.Separator1);

    AssetTagSize -= sizeof (CHAR16);
    for (c = 0; c <  sizeof (T234CvmEeprom->PartNumber.Sku); c++) {
      UnicodeSPrint(&MiscData->BoardAssetTag[CharCount++],
                    AssetTagSize,
                    L"%c", T234CvmEeprom->PartNumber.Sku[c]);
    }

    AssetTagSize -= (c * sizeof (CHAR16));
    UnicodeSPrint(&MiscData->BoardAssetTag[CharCount++],
                  AssetTagSize,
                  L"%c", T234CvmEeprom->PartNumber.Separator2);

    AssetTagSize -= sizeof (CHAR16);
    for (c = 0; c <  sizeof (T234CvmEeprom->PartNumber.Fab); c++) {
      UnicodeSPrint (&MiscData->BoardAssetTag[CharCount++],
                    AssetTagSize,
                    L"%c", T234CvmEeprom->PartNumber.Fab[c]);
    }

    AssetTagSize -= (c * sizeof (CHAR16));
    UnicodeSPrint(&MiscData->BoardAssetTag[CharCount++],
                  AssetTagSize,
                  L"%c", T234CvmEeprom->PartNumber.Separator3);

    AssetTagSize -= sizeof (CHAR16);

    UnicodeSPrint(&MiscData->BoardAssetTag[CharCount++],
                  AssetTagSize,
                  L"%c", T234CvmEeprom->PartNumber.Revision);
    AssetTagSize -= sizeof (CHAR16);
    UnicodeSPrint(&MiscData->BoardAssetTag[CharCount++],
                  AssetTagSize,
                  L"%c", T234CvmEeprom->PartNumber.Separator4);

    AssetTagSize -= sizeof (CHAR16);
    UnicodeSPrint(&MiscData->BoardAssetTag[CharCount++],
                  AssetTagSize,
                  L"%c", T234CvmEeprom->PartNumber.Ending);

    AssetTagSize -= sizeof (CHAR16);
    for (c = 0; c <  sizeof (T234CvmEeprom->PartNumber.Pad); c++) {
      UnicodeSPrint(&MiscData->BoardAssetTag[CharCount++],
                    AssetTagSize,
                    L"%c", T234CvmEeprom->PartNumber.Pad[c]);
    }
    AssetTagSize -= (c * sizeof (CHAR16));
  }
  return Status;
Error:
  if (MiscData->BoardSku) {
    FreePool (MiscData->BoardSku);
  }
  if (MiscData->BoardVersion) {
    FreePool (MiscData->BoardVersion);
  }
  if (MiscData->BoardSerialNumber) {
    FreePool (MiscData->BoardSerialNumber);
  }
  if (MiscData->BoardAssetTag) {
